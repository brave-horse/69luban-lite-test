...
#define H5_RX_ACK_TIMEOUT	K_MSEC(30)
#define H5_TX_ACK_TIMEOUT	K_MSEC(30)
...
struct h5_data {
	...
	struct k_sem        tx_ready;

	struct k_sem        setup_ready;
...
static int h5_unslip_byte(struct rt_device *uart, uint8_t *byte)
{
	int count;
...
static uint8_t h5_slip_byte(struct rt_device *uart, uint8_t byte)
{
	switch (byte) {
	case SLIP_DELIMITER:
...
static void h5_send(struct device *dev, const uint8_t *payload, uint8_t type, int len)
{
...
static void h5_process_complete_packet(struct device *dev, uint8_t *hdr)
{
	struct h5_data *h5 = dev->data;
	struct net_buf *buf;
...
static void bt_uart_isr(const struct rt_device *uart, void *user_data)
{
...
static void tx_thread(void *p1, void *p2, void *p3)
{
	...
	while (true) {
		...
		k_sem_take(&h5->tx_ready, K_FOREVER);
		...
		case INIT:
			...
			break;
		case ACTIVE:
			buf = k_fifo_get(&h5->tx_queue, K_NO_WAIT);
...
static void rx_thread(void *p1, void *p2, void *p3)
{
	...
	while (true) {
		...
		k_sem_take(&h5->rx_ready, K_FOREVER);
		buf = k_fifo_get(&h5->rx_queue, K_NO_WAIT);
...
static int h5_setup(const struct device *dev, const struct bt_hci_setup_params *params)
{
	const struct h5_config *cfg = dev->config;
	struct h5_data *h5 = dev->data;
    int err = 0;
	 ...
	 */
	//extern int bt_h5_vnd_setup(struct device *dev);
	//extern int bt_h5_vnd_setup(struct rt_device *uart);
    extern int bt_h5_vnd_update_controller_baudrate(struct rt_device *uart);
    extern int bt_h5_vnd_update_host_baudrate(struct rt_device *uart);
    extern int bt_h5_vnd_update_host_flowcontrol(struct rt_device *uart);
    extern int bt_h5_vnd_update_controller_firmware(struct rt_device *uart);
	k_sem_take(&h5->setup_ready, K_FOREVER);
    err = bt_h5_vnd_update_controller_baudrate(cfg->uart);
    if (err) {
        pr_err("bt_h5_vnd_update_controller_baudrate failed\n");
        return err;
    }
	h5_send(dev, NULL, HCI_3WIRE_ACK_PKT, 0);
    k_msleep(250);

    err = bt_h5_vnd_update_host_baudrate(cfg->uart);
    if (err) {
        pr_err("bt_h5_vnd_update_host_baudrate failed\n");
        return err;
    }

    err = bt_h5_vnd_update_host_flowcontrol(cfg->uart);
    if (err) {
        pr_err("bt_h5_vnd_update_host_flowcontrol failed\n");
        return err;
    }

    err = bt_h5_vnd_update_controller_firmware(cfg->uart);
    if (err) {
        pr_err("bt_h5_vnd_update_controller_firmware failed\n");
        return err;
    }
	/* Send HCI_RESET */
    ...
    return err;
	//return bt_h5_vnd_setup(cfg->uart);
}
#endif
...
static K_KERNEL_STACK_DEFINE(rx_thread_stack, (CONFIG_BT_DRV_RX_STACK_SIZE * 2));
static K_KERNEL_STACK_DEFINE(tx_thread_stack, (CONFIG_BT_DRV_TX_STACK_SIZE * 2));
static struct k_thread h5_rx_thread;
static struct k_thread h5_tx_thread;

static struct h5_config h5_driver_config = {
	//.uart = DEVICE_DT_GET(DT_INST_PARENT(0)),
	.rx_stack = rx_thread_stack,
	.rx_stack_size = K_KERNEL_STACK_SIZEOF(rx_thread_stack),
	.rx_thread = &h5_rx_thread,
	.tx_stack = tx_thread_stack,
	.tx_stack_size = K_KERNEL_STACK_SIZEOF(tx_thread_stack),
	.tx_thread = &h5_tx_thread,
};

static struct h5_data h5_driver_data = {
    .tx_queue = Z_FIFO_INITIALIZER(h5_driver_data.tx_queue),
    .rx_queue = Z_FIFO_INITIALIZER(h5_driver_data.rx_queue),
    .unack_queue = Z_FIFO_INITIALIZER(h5_driver_data.unack_queue),
};

//DT_INST_FOREACH_STATUS_OKAY(BT_UART_DEVICE_INIT)
int h5_hci_init(void)
{
    static struct device bt_dev;
    int ret = 0;

    rt_memset(&bt_dev.parent, 0, sizeof(struct rt_device));

    h5_driver_config.uart = rt_device_find(LPKG_BT_UART);

	k_fifo_init(&h5_driver_data.tx_queue);
	k_fifo_init(&h5_driver_data.rx_queue);
	k_fifo_init(&h5_driver_data.unack_queue);

	k_sem_init(&h5_driver_data.rx_ready, 0, 1);
	k_sem_init(&h5_driver_data.tx_ready, 0, 1);
	k_sem_init(&h5_driver_data.setup_ready, 0, 1);

    bt_dev.api = &h5_driver_api;
    bt_dev.config = &h5_driver_config;
    bt_dev.data = &h5_driver_data;
    bt_dev.parent.type = RT_Device_Class_Unknown;

    /* Register device */
    ret = rt_device_register(&bt_dev.parent, "hci", RT_DEVICE_FLAG_RDWR);

    return ret;
}
INIT_DEVICE_EXPORT(h5_hci_init);
