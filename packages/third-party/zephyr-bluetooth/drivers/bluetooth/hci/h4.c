...
static size_t h4_discard(const struct rt_device *uart, size_t len)
{
	uint8_t buf[33];
	int err;
...
/** Setup the HCI transport, which usually means to reset the Bluetooth IC
  *
  * @param dev The bt_hci_device structure for the bus connecting to the IC
  *
  * @return 0 on success, negative error value on failure
  */
int __weak bt_hci_transport_setup(const struct rt_device *uart)
{
	h4_discard(uart, 32);
	return 0;
}
...
static int h4_open(const struct device *dev, bt_hci_recv_t recv)
{
    ...
    //rt_device_set_rx_indicate(cfg->uart, serial_input_mq);
...
#define BT_UART_DEVICE_INIT(inst) \
	static K_KERNEL_STACK_DEFINE(rx_thread_stack_##inst, BT_DRV_RX_STACK_SIZE); \
	...
	DEVICE_DT_INST_DEFINE(inst, NULL, NULL, &h4_data_##inst, &h4_config_##inst, \
			      POST_KERNEL, KERNEL_INIT_PRIORITY_DEVICE, &h4_driver_api)

//int h4_init(void)
//{
//	static K_KERNEL_STACK_DEFINE(rx_thread_stack_0, BT_DRV_RX_STACK_SIZE);
//	static struct k_thread rx_thread_0;
//	static const struct h4_config h4_config_0 = {
//		//.uart = DEVICE_DT_GET(DT_INST_PARENT(0)),
//		.rx_thread_stack = rx_thread_stack_0,
//		.rx_thread_stack_size = K_KERNEL_STACK_SIZEOF(rx_thread_stack_0),
//		.rx_thread = &rx_thread_0,
//	};
//
//	static struct h4_data h4_data_0 = {
//		.rx = {
//			.fifo = Z_FIFO_INITIALIZER(h4_data_0.rx.fifo),
//		},
//		.tx = {
//			.fifo = Z_FIFO_INITIALIZER(h4_data_0.tx.fifo),
//		},
//	};
//	//DEVICE_DT_INST_DEFINE(0, NULL, NULL, &h4_data_0, &h4_config_0,
//	//		      POST_KERNEL, KERNEL_INIT_PRIORITY_DEVICE, &h4_driver_api)
//	//DEVICE_DT_INST_DEFINE(0, NULL, NULL, &h4_data_0, &h4_config_0,
//	//		      3, 50, &h4_driver_api);
//
...
static K_KERNEL_STACK_DEFINE(rx_thread_stack, LPKG_BT_DRV_RX_STACK_SIZE);
static struct k_thread h4_rx_thread;

static struct h4_config h4_driver_config = {
	//.uart = DEVICE_DT_GET(DT_INST_PARENT(0)),
	.rx_thread_stack = rx_thread_stack,
	.rx_thread_stack_size = K_KERNEL_STACK_SIZEOF(rx_thread_stack),
	.rx_thread = &h4_rx_thread,
};

static struct h4_data h4_driver_data = {
	.rx = {
		.fifo = Z_FIFO_INITIALIZER(h4_driver_data.rx.fifo),
	},
	.tx = {
		.fifo = Z_FIFO_INITIALIZER(h4_driver_data.tx.fifo),
	},
};
...
int h4_init(void)
{
    ...
    h4_driver_config.uart = rt_device_find(LPKG_BT_UART);
    k_fifo_init(&h4_driver_data.rx.fifo);
	k_sem_init(&h4_driver_data.rx.ready, 0, 1);
    k_fifo_init(&h4_driver_data.tx.fifo);

    bt_dev.api = &h4_driver_api;
    bt_dev.config = &h4_driver_config;
    bt_dev.data = &h4_driver_data;
