...
/* Do not let a stuck bitstream request starve the UI and network threads. */
#define JPEG_BITREQ_TIMEOUT_MS 100
...
static int ve_config_bitstream_register(struct mjpeg_dec_ctx *s, int offset, int is_last)
{
    int busy = 1;
    int timeout = JPEG_BITREQ_TIMEOUT_MS;
    int bit_offset = 0;
    /* The VE requires EOS padding: V1.0 needs one extra 256-byte block,
     * while older revisions need two. */
    int stream_num = 0;
    ...
    if (busy == 1) {
        loge("bitstream request timeout, busy: %d, offset: %d, size: %lu",
             busy, offset, (unsigned long)s->curr_packet->size);
        return -1;
    }

#ifndef AIC_VE_DRV_V10
    timeout = JPEG_BITREQ_TIMEOUT_MS;
    write_reg_u32(s->regs_base + JPG_REQ_REG, 1);
    do {
        busy = read_reg_u32(s->regs_base + JPG_BUSY_REG);
        if (busy != 1)
    ...
    if (busy == 1) {
        loge("second bitstream request timeout, busy: %d", busy);
        return -1;
    }
#endif
...
int ve_decode_jpeg(struct mjpeg_dec_ctx *s, int byte_offset)
{
	...
	// 6. config bitstream
	if (ve_config_bitstream_register(s, byte_offset, 1) < 0) {
		ve_reset();
		ve_put_client();
		return -1;
	}
