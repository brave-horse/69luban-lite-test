...
config LPKG_BT_MPL_MEDIA_PLAYER_NAME
	string "Media Player Name"
	default "Player0"
	help
	  Use this option to set the name of the media player.

config LPKG_BT_MPL_MEDIA_PLAYER_NAME_MAX
...
config LPKG_BT_MPL_ICON_URL_MAX
...
config LPKG_BT_MPL_TRACK_TITLE_MAX
...
config LPKG_BT_MPL_SEGMENT_NAME_MAX
...
config LPKG_BT_MPL_GROUP_TITLE_MAX
	int "Max length of the title of a group of tracks"
	default LPKG_BT_MPL_TRACK_TITLE_MAX
	range 1 255
	help
	  Sets the maximum number of bytes (including the null termination) of
	  the title of any group in the media player.

config LPKG_BT_MPL_OBJECTS
	bool "Support for media player objects"
	depends on LPKG_BT_OTS
	# TODO: Temporarily depends also on LPKG_BT_MCS, to avoid issues with the
	# bt_mcs_get_ots() call
	depends on LPKG_BT_MCS
	help
...
if LPKG_BT_MPL_OBJECTS

config LPKG_BT_MPL_MAX_OBJ_SIZE
