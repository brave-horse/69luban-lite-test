...
void (*g_aic_touch_input_hook)(void) = NULL;
...
static void input_read(lv_indev_t *indev_drv, lv_indev_data_t *data)
{
    ...
    // if (last_x != last_point.x || last_y != last_point.y)
    // {
    //     // printf
    // }


    if (last_state == LV_INDEV_STATE_PRESSED && g_aic_touch_input_hook)
    {
        g_aic_touch_input_hook();
    }
}
