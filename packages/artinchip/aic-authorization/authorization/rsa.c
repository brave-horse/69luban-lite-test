...
{
    if (ctx->alg_ctx.rsa.n)
        aicos_free_align(0, ctx->alg_ctx.rsa.n);
    if (ctx->alg_ctx.rsa.d)
        aicos_free_align(0, ctx->alg_ctx.rsa.d);
    if (ctx->alg_ctx.rsa.e)
        aicos_free_align(0, ctx->alg_ctx.rsa.e);
    ctx->alg_ctx.rsa.n = NULL;
    ctx->alg_ctx.rsa.e = NULL;
    ctx->alg_ctx.rsa.d = NULL;
}

static int aic_akcipher_rsa_unprepare_req(struct akcipher_tfm *tfm,
                                          struct akcipher_request *req)
{
    ...
    if (rctx->ssram_addr) {
        aic_ssram_free(rctx->ssram_addr, ctx->alg_ctx.rsa.n_sz * 3);
        rctx->ssram_addr = 0;
    }
...
static int aic_akcipher_rsa_prepare_req(struct akcipher_tfm *tfm,
                                        struct akcipher_request *req)
{
    struct aic_akcipher_tfm_ctx *ctx;
    struct aic_akcipher_req_ctx *rctx;
    ...
    key_size = rsa_key_size(ctx->alg_ctx.rsa.n_sz);
    ...
    if (ctx->alg_ctx.rsa.d)
        hal_crypto_bignum_be2le(ctx->alg_ctx.rsa.d, ctx->alg_ctx.rsa.d_sz, p, key_size);
    if (ctx->alg_ctx.rsa.e)
        hal_crypto_bignum_be2le(ctx->alg_ctx.rsa.e, ctx->alg_ctx.rsa.e_sz, p, key_size);
    hal_crypto_bignum_be2le(ctx->alg_ctx.rsa.n, ctx->alg_ctx.rsa.n_sz, n, key_size);
...
err:
    aic_akcipher_rsa_unprepare_req(tfm, req);
    return ret;
}

static int aic_akcipher_rsa_do_one_req(struct akcipher_request *req)
{
    struct aic_akcipher_req_ctx *rctx;
                                  ...
                                  unsigned long flag)
{
    ...
    ret = aic_akcipher_rsa_prepare_req(tfm, req);
    if (ret) {
        pr_err("akcipher prepare req failed.\n");
        return ret;
    }
    ret = aic_akcipher_rsa_do_one_req(req);
    if (ret) {
        pr_err("akcipher do one req failed.\n");
        return ret;
    }
    ret = aic_akcipher_rsa_unprepare_req(tfm, req);
    if (ret) {
                                 ...
                                 const void *key, unsigned int keylen)
{
    ...
    ctx->alg_ctx.rsa.n = aicos_memdup_align(0, (void *)rsa_key.n,
    ...
    ctx->alg_ctx.rsa.n_sz = rsa_key.n_sz;

    ctx->alg_ctx.rsa.e = aicos_memdup_align(0, (void *)rsa_key.e,
    ...
    ctx->alg_ctx.rsa.e_sz = rsa_key.e_sz;
                                  ...
                                  const void *key, unsigned int keylen)
{
    ...
    ctx->alg_ctx.rsa.d = aicos_memdup_align(0, (void *)rsa_key.d,
    ...
    ctx->alg_ctx.rsa.d_sz = rsa_key.d_sz;
...
u32 aic_akcipher_rsa_max_size(struct aic_akcipher_handle *handle)
{
    ...
    return rsa_key_size(ctx->alg_ctx.rsa.n_sz);
}
