...
#define _LOG_MODULE_CONST_DATA_CREATE(_name, _level)                                               \
    IF_ENABLED(CONFIG_LOG_FMT_SECTION,                                                             \
               (static const char UTIL_CAT(_name, _str)[] __in_section(                            \
                    _log_strings, static, _CONCAT(_name, _)) __used __noasan = STRINGIFY(_name);)) \
    ...
    const STRUCT_SECTION_ITERABLE_ALTERNATE(log_const, log_source_const_data,                      \
        ...
        .name = COND_CODE_1(CONFIG_LOG_FMT_SECTION, (UTIL_CAT(_name, _str)), (STRINGIFY(_name))),  \
        .level = (_level)                                                                          \
    }
