...
#ifdef RT_USING_PM
static int n5_pm_suspend(const struct rt_device *device, rt_uint8_t mode)
{
    return RT_EOK;
}

static void n5_pm_resume(const struct rt_device *device, rt_uint8_t mode)
{
    struct n5_dev *sensor = (struct n5_dev *)device;
...
static struct rt_device_pm_ops n5_pm_ops =
{
    SET_DEVICE_PM_OPS(n5_pm_suspend, n5_pm_resume)
    NULL,
};
#endif /* RT_USING_PM */
...
#ifdef RT_USING_PM
    rt_pm_device_register(&g_n5_dev.dev, &n5_pm_ops);
#endif
