...
static uint8_t *create_draw_buf(aic_disp_t *aic_disp, int w, int h, lv_color_format_t cf)
{
    ...
    if (!aic_disp->buf) {
        ...
        /* 软重启后 CMA 可能保留旧画面，绘图缓冲必须先清成黑色。 */
        memset(aic_disp->buf_aligned, 0, buf_size);
...
static int lv_de_display_init()
{
    ...
    if (aic_disp->fb) {
        ...
        /*
         * 软重启不会保证 framebuffer 内容被硬件清除。
         * 在面板第一次切帧前清空所有显示缓冲，避免露出重启前的页面。
         */
#ifdef AIC_PAN_DISPLAY
        memset(aic_disp->info.framebuffer, 0, fb_size * 2);
        aicos_dcache_clean_range((ulong *)aic_disp->info.framebuffer,
                                 (ulong)ALIGN_UP(fb_size * 2, CACHE_LINE_SIZE));
#else
        memset(aic_disp->info.framebuffer, 0, fb_size);
        aicos_dcache_clean_range((ulong *)aic_disp->info.framebuffer,
