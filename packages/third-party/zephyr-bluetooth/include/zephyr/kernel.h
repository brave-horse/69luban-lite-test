...
#include <zephyr/k_fifo.h>
#include <zephyr/k_sem.h>
#include <zephyr/k_poll.h>
 ...
 *
 * @note @kconfig{THREAD_MONITOR} must be set for this function
 * to be effective.
 ...
 * If SMP is not defined the implementation this is the same as
 ...
 * running on specified cpu. If SMP is not defined the
 ...
 * No effect if @kconfig{FPU_SHARING} is not enabled.
 */
 ...
 * @kconfig{USERSPACE} is not enabled.
 */
 ...
 * No effect if @kconfig{DSP_SHARING} is not enabled.
 */
#define K_DSP_IDX 6
 ...
 * memory and DSP feature. Often used with @kconfig{ARC_AGU_SHARING}.
 * No effect if @kconfig{ARC_AGU_SHARING} is not enabled.
 */
#define K_AGU_IDX 7
 ...
 * the thread. No effect if @kconfig{X86_SSE} is not enabled.
 */
 ...
 * - @ref K_USER allocate a userspace thread (requires `USERSPACE=y`)
 *
 ...
 *   threads only. These stacks use less memory if USERSPACE is
 *   enabled.
 *
...
#if defined(INIT_STACKS) && defined(THREAD_STACK_INFO)
/**
 ...
 * on a platform which selects @kconfig{NO_UNUSED_STACK_INSPECTION}, an
...
static inline int32_t k_sleep(k_timeout_t timeout)
{
    ...
    //return rt_thread_mdelay(timeout * 1000);
 ...
 * the duration of a tick, @kconfig{SYS_CLOCK_TICKS_PER_SEC} must be
 ...
 * clock tolerances.
 *
 * @note In case when @kconfig{SYSTEM_CLOCK_SLOPPY_IDLE} and
 * @kconfig{PM} options are enabled, this function may not work.
 ...
 */
#define k_yield rt_thread_yield
...
static inline void *k_current_get(void)
{
//#ifdef CURRENT_THREAD_USE_TLS
//
...
//#endif
    ...
    //tid->task.handle = rt_thread_self();
...
#ifdef TIMER_READS_ITS_FREQUENCY_AT_RUNTIME
 ...
 *          may need to be multiples of MMU_PAGE_SIZE (if MMU)
 ...
 *
 * Priority can be assigned in the range of -NUM_COOP_PRIORITIES to
 * NUM_PREEMPT_PRIORITIES-1, where -NUM_COOP_PRIORITIES is the
 * highest priority.
 *
 ...
 * logic.
 *
 * @note You should enable @kconfig{SCHED_DEADLINE} in your project
 * configuration.
 *
 ...
 *
 * @note You should enable @kconfig{SCHED_CPU_MASK} in your project
 * configuration.
 *
 ...
 * TIMESLICE_PER_THREAD=y) sets the timeslice for a specific
 ...
 * not prevent threads from running on other CPUs when SMP=y.
 ...
 *
 * Set the name of the thread to be used when @kconfig{THREAD_MONITOR}
 ...
 * ticks (c.f. @kconfig{SYS_CLOCK_TICKS_PER_SEC}), which is the
 ...
 *    @kconfig{SYS_CLOCK_TICKS_PER_SEC} config option.
 *
...
// *
// * @see TIMER_HAS_64BIT_CYCLE_COUNTER
// *
// * @return Current hardware clock up-counter (in cycles).
// */
//static inline uint64_t k_cycle_get_64(void)
//{
//	if (!IS_ENABLED(TIMER_HAS_64BIT_CYCLE_COUNTER)) {
 ...
 *	     to block again.
 */
int k_futex_wait(struct k_futex *futex, int expected,
			   k_timeout_t timeout);
 ...
 */
uint32_t k_event_set_masked(struct k_event *event, uint32_t events,
 ...
 * @retval -EAGAIN Waiting period timed out.
 */
int k_stack_pop(struct k_stack *stack, stack_data_t *data,
			  k_timeout_t timeout);
 ...
 */
__deprecated int k_pipe_alloc_init(struct k_pipe *pipe, size_t size);
 ...
 */
__deprecated int k_pipe_put(struct k_pipe *pipe, const void *data,
 ...
 */
__deprecated  int k_pipe_get(struct k_pipe *pipe, void *data,
 ...
 */
__deprecated  size_t k_pipe_read_avail(struct k_pipe *pipe);
 ...
 *         result is zero for unbuffered pipes.
 */
__deprecated size_t k_pipe_write_avail(struct k_pipe *pipe);
 ...
 * @param pipe Address of the pipe.
 */
__deprecated void k_pipe_flush(struct k_pipe *pipe);
 ...
 * @param pipe Address of the pipe.
 */
__deprecated void k_pipe_buffer_flush(struct k_pipe *pipe);
...
// *
// * @note @a timeout must be set to K_NO_WAIT if called from ISR.
// * @note When MULTITHREADING=n any @a timeout is treated as K_NO_WAIT.
// *
// * @funcprops \isr_ok
// *
...
#if !defined(ASSERT_NO_FILE_INFO)
