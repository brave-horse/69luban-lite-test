...
static int bt_power_pin = 0;
static int bt_reset_pin = 0;
...
static int realtek_bt_reset(void)
{
    unsigned int g;
    unsigned int p;
...
int bt_ctlr_init(void)
{
#ifndef AIC_WIRELESS_PWR_GPIO
    bt_power_pin = hal_gpio_name2pin(AIC_WIRELESS_BT_PWR_GPIO);
    if (bt_power_pin < 0) {
        pr_err("Not found bt power(%s) pin.\n", AIC_WIRELESS_BT_PWR_GPIO);
        return -1;
    }
    hal_gpio_direction_output(GPIO_GROUP(bt_power_pin), GPIO_GROUP_PIN(bt_power_pin));
    bt_power_on();
#endif
#ifdef AIC_WIRELESS_PWR_GPIO
    bt_reset_pin = hal_gpio_name2pin(AIC_WIRELESS_BT_PWR_GPIO);
    if (bt_reset_pin < 0) {
