...
#ifdef RT_USING_PM
static int gc032a_pm_suspend(const struct rt_device *device, rt_uint8_t mode)
{
    return RT_EOK;
}

static void gc032a_pm_resume(const struct rt_device *device, rt_uint8_t mode)
{
...
static struct rt_device_pm_ops gc032a_pm_ops =
{
    SET_DEVICE_PM_OPS(gc032a_pm_suspend, gc032a_pm_resume)
    NULL,
};
#endif /* RT_USING_PM */
...
#ifdef RT_USING_PM
    rt_pm_device_register(&g_gc03_dev.dev, &gc032a_pm_ops);
#endif
