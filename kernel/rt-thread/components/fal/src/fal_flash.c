...
/* flash device table */
    ...
    /* User-defined flash device table from fal_cfg.h */
...
#else
    /* FAL flash table managed by platform code (e.g., spinor_fal.c) */
    extern struct fal_flash_dev **fal_flash_table_ptr;
    extern size_t fal_flash_devices_count;

    /* Alias to unify access interface regardless of configuration method */
    #define device_table fal_flash_table_ptr
    #define device_table_len fal_flash_devices_count
#endif
...
int fal_flash_init(void)
{
    ...
    for (i = 0; i < device_table_len; i++)
    {
        ...
        if (device_table[i]->ops.init) {
            device_table[i]->ops.init((struct fal_flash_dev *)device_table[i]);
        }
