...
int aic_get_os_to_startup(char *target_os)
{
    ...
    /* 单 data 分区没有 data_partname_r，也不需要 data A/B 启动状态。 */
    data_partname_r = fw_getenv("data_partname_r");
    if (data_partname_r == NULL || data_partname_r[0] == '\0')
        goto flush_env;

    /* 双 data 分区才读取并同步 data A/B 启动状态。 */
    next = fw_getenv("dataAB_next");
    now = fw_getenv("dataAB_now");
    if (next == NULL || now == NULL || next[0] == '\0' || now[0] == '\0') {
        ret = -1;
        pr_err("data A/B environment is incomplete\n");
        goto err_write_next_os;
    }
#ifdef AIC_ENV_DEBUG
