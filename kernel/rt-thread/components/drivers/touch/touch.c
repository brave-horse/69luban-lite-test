...
static rt_err_t rt_touch_irq_init(rt_touch_t touch)
{
    ...
    {
#ifdef AIC_TOUCH_PANEL_JD9366
        /* JD9366 的 INT 在触摸数据未读完时持续拉低，必须使用低电平触发。 */
        rt_pin_attach_irq(touch->config.irq_pin.pin, PIN_IRQ_MODE_LOW_LEVEL, touch_irq_callback, (void *)touch);
#else
