...
char *ota_upgrade_get_partname(char *file_name)
{
    ...
    if (strncmp("rodata.fatfs", file_name, 12) == 0) {
        ...
        /* 单 data 分区保存用户数据，默认 OTA 包不允许覆盖该分区。 */
        now = fw_getenv("dataAB_now");
        data_partname_r = fw_getenv("data_partname_r");
        if (now == NULL || now[0] == '\0' ||
            data_partname_r == NULL || data_partname_r[0] == '\0') {
            LOG_E("data.fatfs is not supported on a single data partition");
            ret = NULL;
        } else if (strncmp(now, "A", 1) == 0) {
            /* 双 data 分区时，将新 data 镜像写入备用 B 分区。 */
            ...
            /* 双 data 分区当前为 B 时，将新 data 镜像写回 A 分区。 */
