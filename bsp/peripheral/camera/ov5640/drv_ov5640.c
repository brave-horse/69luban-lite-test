...
#ifdef RT_USING_PM
static int ov5640_pm_suspend(const struct rt_device *device, rt_uint8_t mode)
{
    return RT_EOK;
}

static void ov5640_pm_resume(const struct rt_device *device, rt_uint8_t mode)
{
    struct ov5640_dev *sensor = (struct ov5640_dev *)device;
...
static struct rt_device_pm_ops ov5640_pm_ops =
{
    SET_DEVICE_PM_OPS(ov5640_pm_suspend, ov5640_pm_resume)
    NULL,
};
#endif /* RT_USING_PM */
...
#ifdef RT_USING_PM
    rt_pm_device_register(&g_ov5640_dev.dev, &ov5640_pm_ops);
#endif
