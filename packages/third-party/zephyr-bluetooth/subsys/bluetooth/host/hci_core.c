...
//BUILD_ASSERT(IS_ENABLED(ZTEST), "Missing DT chosen property for HCI");
//static struct rt_device bt_hci_dev;
//static struct bt_device bt_hci_dev;

//#define BT_HCI_DEV    ((struct device *)rt_device_find("hci"))
...
int bt_hci_cmd_send_sync(uint16_t opcode, struct net_buf *buf,
			 struct net_buf **rsp)
{
	static struct k_sem sync_sem = { 0 };
	uint8_t status;
	int err;
...
int bt_enable(bt_ready_cb_t cb)
{
    ...
    bt_dev.hci = (const struct device *)rt_device_find("hci");
...
static int bt_tx_processor_init(void)
{
	...
	k_work_queue_start(&bt_tx_processor_workq, bt_tx_processor_stack,
			   ...
			   //BT_TX_PROCESSOR_THREAD_PRIO, &cfg);
			   21, &cfg);
...
INIT_DEVICE_EXPORT(bt_tx_processor_init);
#endif /* BT_TX_PROCESSOR_THREAD */
