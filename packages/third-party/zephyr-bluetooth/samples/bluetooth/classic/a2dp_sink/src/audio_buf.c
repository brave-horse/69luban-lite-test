...
//#if DT_HAS_ALIAS(i2s_codec_tx) && IS_ENABLED(I2S) && IS_ENABLED(AUDIO_CODEC)
//#if IS_ENABLED(I2S) && IS_ENABLED(AUDIO_CODEC)
...
#define A2DP_SBC_DECODER_PCM_BUFFER_SIZE (A2DP_SBC_DATA_PLAY_SIZE_48K *\
					  A2DP_SBC_PCM_BUFFER_PLAY_COUNT)
...
void audio_get_pcm_data(uint8_t **data, uint32_t length)
{
	...
	if (!reach_threshold) {
		if (data_space > A2DP_BOARD_CODEC_PLAY_THRESHOLD * pcm_buffer_size / 100) {
