...
#ifdef RT_USING_PM
static int ov7670_pm_suspend(const struct rt_device *device, rt_uint8_t mode)
{
    return RT_EOK;
}

static void ov7670_pm_resume(const struct rt_device *device, rt_uint8_t mode)
{
    struct ov7670_dev *sensor = (struct ov7670_dev *)device;
...
static struct rt_device_pm_ops ov7670_pm_ops =
{
    SET_DEVICE_PM_OPS(ov7670_pm_suspend, ov7670_pm_resume)
    NULL,
};
#endif /* RT_USING_PM */
...
#ifdef RT_USING_PM
    rt_pm_device_register(&g_ov7670_dev.dev, &ov7670_pm_ops);
#endif
